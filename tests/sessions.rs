...
fn fragmentation_retry_and_fork_all_coalesce_correctly() {
    ...
    assert_eq!(session.records, 15);
    assert_eq!(
        session.messages, 9,
        "5 human turns (a three-way fork) and 4 coalesced assistant messages (the retry is a distinct message)"
    );
}
