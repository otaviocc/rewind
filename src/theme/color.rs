...
impl ColorSpec {
    ...
    pub fn resolve_against(&self, palette: &Palette) -> Result<Color, ColorError> {
        match self {
            ...
            Self::Name(name) => palette.slot(name).or_else(|| literal(name)).ok_or_else(|| ColorError::new(name, ELEMENT_FORMS)),
        }
    }
...
mod tests {
    ...
    fn a_background_can_be_removed_but_a_foreground_cannot() {
        ...
        assert!(!name("reset").removes_color(), "reset is a color: the terminal's own");
        let error = name(NO_COLOR).resolve_against(&palette).expect_err("none is not a color");
