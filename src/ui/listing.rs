//! Selection and scrolling shared by every list column.
...
pub fn target(motion: Motion, selected: usize, last: usize, height: usize) -> usize {
    ...
    match motion {
        ...
        Motion::HalfPage(delta) => step(selected, delta.saturating_mul((height_isize / 2).max(1)), last),
        Motion::Top => 0,
        Motion::Bottom => last,
    }
}
...
pub fn revealed(top: usize, selected: usize, height: usize) -> usize {
    ...
    top.min(selected).max(selected.saturating_sub(height.saturating_sub(1)))
}

pub fn snapped(selected: usize, top: usize, last: usize, height: usize) -> usize {
    let height = height.max(1);
    selected.clamp(top, (top.saturating_add(height).saturating_sub(1)).min(last))
}
