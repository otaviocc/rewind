...
//!
//! Plain OSC 52 depends on the terminal emulator (and, over SSH or inside tmux, on passthrough
//! being configured at all), so it is the fallback rather than the only path. The only place in
//! `ui` allowed to touch the terminal or spawn a process on behalf of a copy — `App` only ever
//! hands text to `App::take_copy`.
