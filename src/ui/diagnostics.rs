...
pub fn lines(drift: &BTreeMap<PathBuf, Diagnostics>, width: usize, theme: &Theme) -> Vec<RenderedLine> {
    ...
    for (path, diagnostics) in drift {
        ...
        lines.push(heading(path, diagnostics, width, theme));
        for defect in diagnostics.defects() {
            lines.push(line(&describe(defect), theme.style(Element::Body), INSET, width));
        }
        let held = diagnostics.defects().len();
        if let Some(beyond) = diagnostics.count().checked_sub(held).filter(|beyond| *beyond > 0) {
            lines.push(line(&format!("… and {beyond} more"), theme.style(Element::Muted), INSET, width));
        }
    }
    lines
}

fn heading(path: &Path, diagnostics: &Diagnostics, width: usize, theme: &Theme) -> RenderedLine {
    ...
    rendered.push(StyledSpan::new(truncate(&name, width.saturating_sub(tally.width())), theme.style(Element::Label)));
    rendered.push(StyledSpan::new(truncate(&tally, width), theme.style(Element::StatusNotice)));
    rendered
}
...
mod tests {
    ...
    fn one_defect_is_not_pluralized() {
        let rendered = lines(&drift(vec![Defect::Truncated { line: 2 }]), 80, &theme());
    ...
    fn the_defects_beyond_the_stored_limit_are_announced_rather_than_dropped_silently() {
        ...
        let rendered = texts(&lines(&store, 80, &theme()));
    ...
    fn a_narrow_pane_truncates_rather_than_overflows() {
        let rendered = lines(&drift(vec![Defect::UnknownBlock { line: 12, kind: "server_tool_use".to_owned() }]), 20, &theme());
