...
- **A transcript file holds two disjoint record schemas, not one envelope with optional
  ...
  `continued-in`, `fork-context-ref`, `bridge-session`, `artifact-*` — carry **only**
  `sessionId`, and sometimes `timestamp`. `file-history-snapshot` and `file-history-delta` carry no
